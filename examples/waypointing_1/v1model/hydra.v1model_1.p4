...
control checkerControl(inout hydra_header_t hydra_header,
  ...
  apply
    ...
    bit<16> waypoint = 3;
    bit<16> switch;
    if (hydra_header.paths[0].isValid())
      {
      switch = hydra_header.paths[0];
      if (waypoint==switch) {
        found = true;
      }
      if (hydra_header.paths[1].isValid())
        {
        switch = hydra_header.paths[1];
        if (waypoint==switch) {
          found = true;
        }
        if (hydra_header.paths[2].isValid())
          {
          switch = hydra_header.paths[2];
          if (waypoint==switch) {
            found = true;
          }
        }
